...
8並列の実行が一番はやくなる．
さらに並列数を増やすと実行時間が長くなる．
これは，ノード間での通信による時間が影響していると考えられる．
%
また，ver.1 に比べ，ver.2 が約4.6倍はやくなる．
...
\begin{figure}[ht]
 ...
 \end{center}
 \caption{\footnotesize{ver.1 とver.2 の実行時間の比較(T213)．左 : 実行時間．ver.1 が赤線，ver.2 (スレッド数は1) が青線．
右 : ver.1 / ver.2 .
}
}
 \label{para_1}
...
\begin{figure}[ht]
 ...
 \end{center}
 \caption{\footnotesize{正規化した誤差(T213, 並列数 : ip = 32)，上から質量，全エネルギー，ポテンシャルエンストロフィー．
...
\begin{figure}[ht]
 ...
 \end{center}
 \caption{\footnotesize{正規化した誤差(T213, 並列数 : ip = 32)，上から渦度，発散．
