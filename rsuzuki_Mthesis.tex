...
%
初期に上層は静止しており，$10 \tau_{interval}$ 経過する間に
最大で50 個の雷雲を想定した質量強制がランダムに加えられる．
%
惑星半径と自転角速度は木星の値を用いる．
また，計算時間は3000 地球日とする．
%
...
\begin{table}[t]
  ...
  \label{table1}
  \begin{center}
    \includegraphics[clip,width=8cm]{./fig/model/table1.png}
  \end{center}
\end{table}
%
表\ref{table2}に計算を行った数値実験のケースとその用いたパラメータを示す．

以下ではA3 のケースを標準実験とし，\ref{sec:A3}節でその実験結果について述べる．
%
その次に，\ref{sec:case1}節では$g'h_{eq}$の値を変化させ，Burger 数の影響を調べた実験1の結果について示す．
\ref{sec:case2}節では$\tau_{APE}$の値を変化させ，放射緩和の影響を調べた実験2の結果について示す．
...
\begin{table}[t]
  ...
  \label{table2}
  \begin{center}
    \includegraphics[clip,width=20cm]{./fig/model/table2.png}
  \end{center}
\end{table}
\end{landscape}
%
...
\section{標準実験結果 : A3}
\label{sec:A3}
ここでは，標準実験とした A3 のケースの結果を図\ref{fig:A3_1}-\ref{fig:A3_5}に示す．
用いたパラメータは表\ref{table2}を参照．
図\ref{fig:A3_1}の東西風速を見てわかるように，
赤道域で幅が広く，風速が約$220 \rm{~m \cdot s^{-1}}$と強力な東風ジェットと
緯度約$\pm 30^\circ$ で風速約$100 \rm{~m \cdot s^{-1}}$の西風ジェットが見られる．
%
また，その西風ジェットに対応して，図\ref{fig:A3_2}のジオポテンシャルが
その領域で値が小さくなっている．
%
...
図\ref{fig:A3_3}の相対渦度分布を見て分かるように，
高気圧性渦が見て取れる．
...
$Q_e^*<0$は高気圧性の渦度を表す．
%
図\ref{fig:A3_4}の極域を見てみると巨大な低気圧性渦の中に，複数の高気圧性渦が形成していることがわかる．
%
図\ref{fig:A3_5}に有効ポテンシャルエネルギー : APE と運動エネルギー : KE 
\begin{align}
...
%
A3 のケースでは$\tau_{APE} = \infty$ のため，APE とKE は時間とともに増加する．
\begin{figure}[b]
  \begin{center}
    \includegraphics[clip,width=10cm]{./fig/result/A3/A3_1.png}
    \caption{
      \footnotesize{A3の実験結果. 左 : 東西風速，右 : 東西平均した東西風速(3000 地球日)．
      }
    }
    \label{fig:A3_1}
  \end{center}
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[clip,width=12cm]{./fig/result/A3/A3_2.png}
    \caption{
      \footnotesize{A3の実験結果. ジオポテンシャル(3000 地球日)．
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[clip,width=12cm]{./fig/result/A3/A3_3.png}
    \caption{
      \footnotesize{A3の実験結果. 相対渦度(3000 地球日)．
左 : 緯度$0^\circ$，経度$0^\circ$から見た球面投影.
中 : 緯度$90^\circ$，経度$0^\circ$から見た球面投影.
右 : 緯度$-90^\circ$，経度$0^\circ$から見た球面投影.
      }
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[clip,width=12cm]{./fig/result/A3/A3_4.png}
    \caption{
      \footnotesize{A3の実験結果. 無次元ポテンシャル渦度(3000 地球日)．
左 : 緯度$0^\circ$，経度$0^\circ$から見た球面投影.
中 : 緯度$90^\circ$，経度$0^\circ$から見た球面投影.
右 : 緯度$-90^\circ$，経度$0^\circ$から見た球面投影.
      }
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[clip,width=12cm]{./fig/result/A3/A3_5.jpg}
    \caption{
      \footnotesize{A3の実験結果. 有効ポテンシャルエネルギー : APE(実線) と運動エネルギー : KE(点線) の時間発展．
      }
    }
    \label{fig:A3_5}
  \end{center}
